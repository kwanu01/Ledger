# Ledger 배포 가이드 — v54 → v69

배포되어 있는 것은 **v54**이고, 이 코드는 **v69**입니다. 그 사이 열다섯 판이
...
git add -A
git commit -m "v69 — 내보내기(CSV): 사람마다 한 칸"
git push
```
...
13. **1인당 회비를 지우고** 다시 보기 — 비워 두어도 회비 납부 표가 그대로
    서는가. "3명 중 2명이 이 금액을 냈기에 기준으로 삼았습니다"가 뜨는가.
14. **수입 한 줄로 적기** — "현우 3월 회비 3만원" 을 넣어 보기. 갈래(회비)와
    낸 사람이 자동으로 골라지는가.
15. **확인할 것** — 같은 금액·같은 결제자로 같은 날 두 줄을 적어 보기.
    장부 화면 위에 띠가 뜨고, 검은 막대가 깜빡이는가. '괜찮습니다'를 누르면
    사라지고, **새로고침해도 안 돌아오는가**(0021 이 안 돌았으면 여기서 실패).
16. **결산 보고서** — 장부 맨 아래 링크. 결산 식과 표가 나오고,
    **인쇄 미리보기에서 탭 줄과 수증이가 안 보이는가**.
17. **말 대신 써 주기** — 회비 미납자 옆 단추. 두세 문장이 나오고 금액이
    **화면의 모자란 금액과 같은지**. 말투를 바꿔 다시 써 보기.
18. **내보내기** — 장부 맨 아래 '지출 내려받기'. 엑셀에서 열어
    **한글이 안 깨지는지**, 그리고 **사람 칸을 가로로 더하면 금액과 같은지**.
    환불(음수) 줄이 있으면 그 줄도 더해지는지 꼭 보세요.
19. **집행률** — 결산 화면의 띠와 백분율. 예산을 안 적었으면 '들어온 돈에서
    셈'이라고 적히는가. '고치기'로 예산을 넣으면 그 값이 이기는가.
    (0022 가 안 돌았으면 예산 저장에서 실패)

3·5·12·15 번이 이번 판의 핵심이고, **8번과 12번이 회계가 안 깨졌다는
증거**입니다. 15·18 번은 마이그레이션 0021·0022 가 제대로 돌았는지를
화면에서 확인하는 자리이기도 합니다.

### 빈 장부로도 한 번

새 장부를 하나 만들어 **아무것도 안 적은 채로** 모든 탭을 눌러 봅니다.
새로 만든 계산들(검사·예산·말 걸기)은 전부 "말할 근거가 모자라면 아무 말도
안 한다"는 규칙 위에 서 있어서, 빈 장부에서 조용한 것이 정상입니다.
무언가 뜨거나 오류가 나면 그것이 버그입니다.

(코드 쪽으로는 `node --experimental-strip-types` 로 빈 장부·모서리 값 22가지를
확인해 두었습니다 — 0으로 나누기, 시작일보다 앞선 오늘, 수입 없는 공금 장부,
정산 날짜가 미래인 경우까지.)
...
| v68 | **수증이가 v55~v67을 전부 알게** 됨(검사·예산·미룬 것·보고서, 화면과 같은 숫자) · 경고 띠에서 **붉은색을 빼고 깜빡임으로** · 칩 줄바꿈 고침 · README 갱신 |
| v69 | **내보내기(CSV)** — 사람마다 한 칸이라 가로로 더하면 금액, 세로로 더하면 부담액. BOM·수식 차단·RFC 5987 파일 이름 |
...
검산 불변식은 **22개 → 143개**로 늘었고 전부 통과합니다 (`npm run simulate`).
