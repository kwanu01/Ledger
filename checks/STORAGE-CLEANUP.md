...
- 유지되는 공동 장부: 회계 기록에 붙은 사진은 공동 기록으로 남는다. 사용자가 탈퇴하는 동안 진행 중인 업로드는 user/member에 결합한 `image_upload_operations`로 추적한다. guest 업로드 뒤 계정 claim도 포함한다. 새 파일 연결은 현재 멤버 권한과 이전 사진 경로를 DB에서 다시 확인한다.
