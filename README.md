...
인터페이스는 Pretendard Variable 하나로 통일합니다. 숫자는 같은 글꼴의 고정폭 숫자
기능을 사용하고, teamLedger 로고만 Mrs Saint Delafield를 바탕으로 만든 고정 벡터
윤곽을 씁니다. 그래서 본문에 명조체가 섞이지 않고, 로고는 기기 글꼴과 상관없이 늘
같은 형태로 보입니다. 로고 오른쪽 아래에는 색을 쓰지 않은 작은 바코드만 둡니다.
