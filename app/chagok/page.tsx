...
export const metadata: Metadata = {
  ...
  openGraph: { title, description, url: 'https://teamledger.net/chagok', siteName: '차곡', locale: 'ko_KR', type: 'website' },
};
...
const questions = [
  ...
  ['차곡에서 실제로 돈을 보내나요?', '차곡은 누가 누구에게 얼마를 보낼지 계산하고 기록합니다. 실제 송금은 사용하던 은행 앱에서 진행해 주세요. 보냄·받음 표시는 사용자가 확인해 남기는 기록이에요.'],
...
export default function Chagok() {
  return <div className={styles.page} lang="ko">
    ...
    <header className={styles.nav}>
      <Link className={`${styles.wordmark} ${script.className}`} href="/chagok" aria-label="차곡 홈">chagok</Link>
    ...
    <main id="chagok-main" className={styles.main}>
      <section className={styles.hero} aria-labelledby="chagok-title">
        <div className={styles.intro}>
          ...
          <h1 id="chagok-title">함께 쓰는 장부,<br />차곡.</h1>
        ...
        <figure className={styles.preview} aria-label="세 사람이 함께 쓴 돈의 기록 예시">
          <div className={styles.paper}>
            <div className={styles.paperHead}><span className={`${styles.paperMark} ${script.className}`}>chagok</span><span>기록 예시</span></div>
            ...
            <div className={styles.paperFoot}><span>계산까지, 차곡.</span><span className={styles.barcode} aria-hidden="true" /></div>
          </div>
      ...
      <section className={styles.features} aria-label="차곡으로 정리하는 세 가지">
