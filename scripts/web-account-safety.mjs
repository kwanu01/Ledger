...
// Candidate merge keeps the manual Apple cleanup branch inside try/finally.
const manual = client(B, async () => ({ ok: true, value: { done: true, removedBooks: 0, appleCleanup: 'manual_required' } }));
const manualNavigation = navigation.length;
click(manual); click(manual); await manual.settle();
const manualTree = manual.render();
assert.ok(find(manualTree, node => node.props?.role === 'status'));
assert.ok(find(manualTree, node => node.type === 'a' && node.props?.href === 'https://account.apple.com/'));
assert.equal(find(manualTree, node => node.type === 'button'), null);
assert.equal(navigation.length, manualNavigation, 'manual Apple cleanup remains visible instead of immediate navigation');
manual.unmount();

// Execute the actual repository against a conditional-write fixture.
const T = 'team-1';
let owner = A, readError = null, updateError = null, rows = 0;
const member = { user_id: B, active: true, team_id: T, account_deleted_at: null };
const db = { from(table) {
  const filters = [], request = { payload: null };
  const query = {
    select(columns) { if (table === 'members') assert.ok(columns.includes('account_deleted_at')); return query; },
...
const teamActions = load('app/actions/teams.ts', {
  ...
  '../../lib/mobile/auth-policy.ts': load('lib/mobile/auth-policy.ts', {}),
